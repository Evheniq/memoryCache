...
func main() {
	jobsCount, workerCount := 100, 5
	rand.Seed(time.Now().Unix())
	startTime := time.Now()
	usersToSave := make(chan User, jobsCount)
	done := make(chan bool, workerCount)

	for i := 0; i < workerCount; i++ {
		go workerSaveUserInfo(usersToSave, done)
	}

	generateUsers(jobsCount, usersToSave)
	close(usersToSave)
...
func workerSaveUserInfo(users <-chan User, done chan<- bool) {
